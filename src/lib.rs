...
//!
//! ```rust
//! use solar_positioning::{time::JulianDate, types::SolarPosition};
...
//!
//! // Example with solar position (when algorithms are implemented)
//! // let position = spa::solar_position(...).expect("Invalid coordinates");
...
mod tests {
    ...
    fn test_basic_spa_calculation() {
        ...
        let position =
            spa::solar_position(datetime, 37.7749, -122.4194, 0.0, 67.0, 1013.25, 15.0).unwrap();

        assert!(position.azimuth() >= 0.0);
        assert!(position.azimuth() <= 360.0);
        assert!(position.zenith_angle() >= 0.0);
        assert!(position.zenith_angle() <= 180.0);
    }
    ...
    fn test_basic_grena3_calculation() {
        ...
        let datetime = "2023-06-21T12:00:00Z".parse::<DateTime<Utc>>().unwrap();
        let position = grena3::solar_position_with_refraction(
            datetime,
            37.7749,
            -122.4194,
            67.0,
